...
4. 그룹별 대표 파일 1개로 스키마 확정
5. 그룹 단위 배치 스캔 수행
6. 그룹 읽기 스키마 오류/실패 또는 과대 그룹 시 파일 단위 폴백
7. 결과/오류 리포트 저장

### 3.2 그룹 전략
- 그룹 키: `directoryPath`, `format`, `schemaSignature`
- 스키마 시그니처는 각 `(directoryPath, format)` 그룹의 대표 파일 1개에서 산출한다
- CSV는 대표 파일의 헤더 순서를 유지한 시그니처를 사용하고, 그룹 일괄 읽기 시 대표 스키마와 헤더를 검증해 컬럼 매핑 왜곡을 방지한다
- JSON/Parquet/ORC 다중 파일 그룹은 배치 읽기 후 파일별 스키마를 재검증해 대표 스키마와 불일치하면 파일 단위 스캔으로 전환한다
...
- 디렉토리 일괄 읽기 이후 파일 단위 폴백에서도 모든 성공 파일의 스키마가 대표 스키마와 일치할 때만 디렉토리 식별자를 유지한다
...
## 5. 운영 특성
- 로그는 스캔 요약(`scanned_files`, `groups`, `detections`, `errors`)과 폴백 원인/실행 경로를 드라이버 로그에 출력하고, `bin/privyspark-submit`의 `PRIVYSPARK_DEBUG=true` 또는 `spark-submit`의 `spark.yarn.appMasterEnv.PRIVYSPARK_DEBUG=true`/`-Dprivyspark.debug=true`가 설정되면 debug 진행 이벤트(플랜 수립, 대표 스키마 확정, 그룹/파일 스캔, 리포트 저장)를 추가로 출력
