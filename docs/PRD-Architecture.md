...
- stale progress 정리는 shutdown hook이 아니라 다음 실행 시작 시 수행합니다. startup race는 `_progress-preparing.json` lock으로 막고, 준비가 끝나면 active-run marker를 주기 heartbeat로 갱신합니다. active marker가 partial/corrupt 상태가 되더라도 owner run은 `meta/run.json`을 근거로 다음 heartbeat에서 self-heal할 수 있어야 합니다. 반대로 run metadata가 `FAILED`로 전환된 뒤에는 늦게 끝난 병렬 task가 같은 run을 `RUNNING`으로 되살리지 못해야 합니다. `FAILED`이거나 stale heartbeat/stale preparing lock으로 판정되면 cleanup하고, 최근 heartbeat의 `RUNNING` marker나 fresh preparing lock이 남아 있으면 다른 실행의 live progress를 지우지 않도록 즉시 실패합니다.
