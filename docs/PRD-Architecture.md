...
- 따라서 `_progress`를 최종 소스로 사용하는 `runScan` 경로에서는 group scan 반환 row payload를 드라이버에 끝까지 쌓지 않고 즉시 폐기합니다. 최종 결과 집계는 `_progress` merge만 사용합니다.
- stale progress 정리는 shutdown hook이 아니라 다음 실행 시작 시 수행합니다. startup race는 `_progress-preparing.json` lock으로 막고, 준비가 끝나면 active-run marker를 주기 heartbeat로 갱신합니다. active marker가 partial/corrupt 상태가 되더라도 owner run은 `meta/run.json`을 근거로 다음 heartbeat에서 self-heal할 수 있어야 합니다. 반대로 run metadata가 `FAILED`로 전환된 뒤에는 늦게 끝난 병렬 task가 같은 run을 `RUNNING`으로 되살리지 못해야 하며, 다음 실행의 cleanup도 unreadable active marker 뒤의 `FAILED` run metadata를 즉시 stale failure로 정리할 수 있어야 합니다. `FAILED`이거나 stale heartbeat/stale preparing lock으로 판정되면 cleanup하고, 최근 heartbeat의 `RUNNING` marker나 fresh preparing lock이 남아 있으면 다른 실행의 live progress를 지우지 않도록 즉시 실패합니다.
