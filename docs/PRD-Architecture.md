...
- stale progress 정리는 shutdown hook이 아니라 다음 실행 시작 시 수행합니다. active-run marker가 `FAILED`이거나 stale heartbeat로 판정되면 cleanup하고, 최근 heartbeat의 `RUNNING` marker가 남아 있으면 다른 실행의 live progress를 지우지 않도록 즉시 실패합니다.
