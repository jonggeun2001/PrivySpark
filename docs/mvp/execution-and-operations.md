...
- 비정상 종료 시 `_progress`가 남을 수 있으며, 다음 실행 시작 시 active-run marker가 없을 때만 stale `_progress`를 정리합니다.
- active-run marker가 남아 있으면 cleanup 대신 충돌로 실패합니다. 이유는 다른 실행이 아직 `_progress`를 최종 merge 소스로 사용 중일 가능성을 배제할 수 없기 때문입니다.
