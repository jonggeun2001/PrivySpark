...
- `full_column`: 각 비어 있지 않은 값을 regex 전체 일치 기준으로 평가하고, full match된 값 개수를 집계합니다.
- 내부 `text` fallback 포맷에서도 각 줄 전체를 하나의 값으로 보고 `full_column`의 full match 규칙을 그대로 적용합니다.
