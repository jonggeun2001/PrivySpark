...
- CSV는 디렉토리 일괄 읽기 시 대표 헤더를 검증하고, JSON/Parquet/ORC는 배치 후 파일별 스키마를 재검증한다.
- 스키마 검증/읽기 오류가 발생하면 파일 단위로 재확인하고, 모든 성공 파일 스키마가 대표 스키마와 일치할 때만 디렉토리 식별자로 결과를 집계할 수 있다.
