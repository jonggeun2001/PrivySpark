...
- `email`: 토큰 경계를 적용하고, 도메인 라벨 구조를 더 엄격하게 제한해 잘못 붙은 suffix나 비정상 도메인 오탐을 줄입니다.
...
- `driver_license_number`: regex 단계에서 구형 10자리 형식과 현행 12자리 형식을 분리하고, 현행 지역코드는 `11`~`26`, `28`만 허용합니다. 이후 strict validator가 다시 한 번 형식을 확인합니다.
...
- `credit_card_number`: 흔한 16자리 카드 issuer prefix 중심으로 제한하고, 더 긴 숫자 토큰 내부 substring은 제외합니다.
