...
13자리 전체형과 하이픈 있는 7자리 축약형은 기존 숫자 경계를 유지합니다. 이 구분은 `config/rules/default.yaml`의 정규식 분기 안에 있으며, 검출 엔진의 타입별 예외나 ruleset 스키마를 추가하지 않습니다. 커스텀 ruleset은 계속 작성한 정규식 그대로 동작합니다.
