...
- 집계 결과는 `match_count`, `sampled_row_count`, `match_ratio`, `non_null_match_ratio`, `confidence`, `sample_raw_value`, `sample_matched_fragment`를 포함합니다.
- `sample_raw_value`는 매치가 발생한 셀의 전체 원문이 아니라, 매치 조각 기준 앞뒤 최대 50자 문맥만 저장합니다.

## 샘플링과 스캔 단위
- `--sample-ratio`는 row sampling입니다.
- `--file-sample-ratio`는 batch-capable group scan에서 그룹 내 파일을 균등 무작위로 추출합니다.
