...
- probe 대상에서 제외하는 비데이터 확장자와 magic-byte/CSV/text fallback으로 처리할 수 없는 입력은 `Unsupported file format`으로 오류 리포트에 기록합니다.
- 0바이트 physical file은 pre-scan에서 즉시 건너뜁니다.
- pre-scan에서 삭제를 확인한 physical file은 `scan_errors` 없이 건너뜁니다. 확장자로 포맷을 판별해 pre-scan을 통과한 뒤 스키마 검사나 읽기 단계에서 삭제가 확인되면 오류가 기록될 수 있습니다.
...
- 동일 스키마와 오류 없는 범위가 확인된 그룹은 디렉토리 식별자로 승격할 수 있습니다. 입력 루트 아래의 단일 파일 디렉토리도 승격 대상이 될 수 있지만, 입력 루트 자체의 단일 파일과 논리 입력은 승격하지 않습니다. 상세 조건은 [결과 식별자 규칙](reports-and-errors.md#file_identifier-규칙)을 따릅니다.
- archive 엔트리와 Excel 시트는 논리 입력 식별자를 유지합니다.

입력 루트가 Hive Metastore table `LOCATION`과 정확히 같고 ignore 매치가 없으면 별도 table 경로를 사용합니다. 파일 확장/스키마 split 대신 `spark.table`로 읽으며, row sampling만 적용합니다. 자세한 조건은 [Hive table lookup](../operations/execution.md#hive-table-lookup)에 있습니다.
