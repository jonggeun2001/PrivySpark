...
- sampled Parquet, ORC, Avro처럼 batch scan 가능한 non-CSV/non-JSON group은 batch 전에 bounded exact schema validation을 수행하고, validation과 batch scan이 모두 성공하면 파일 식별자를 유지합니다.
- CSV는 fallback 또는 명시적 exact 경로에서 exact split에 도달한 경우 헤더 유무 드리프트도 다시 확인합니다.

스키마 샘플링을 별도 단계로 둔 이유는 두 가지입니다. 첫째, 같은 디렉토리라도 실제 스키마 드리프트가 있을 수 있으므로 디렉토리 단위 집계를 바로 적용하면 식별자 의미가 깨질 수 있습니다. 둘째, 모든 파일을 처음부터 exact split으로 읽으면 pre-scan 비용이 커지므로 대표 파일 샘플링으로 비용과 정확도 사이를 조정하고 non-CSV/non-JSON small-file group은 bounded schema validation 이후 더 저렴한 batch 경로를 사용합니다.
...
- sampled non-CSV/non-JSON batch-capable group은 batch scan 전에 schema validation을 수행하고, validation에서 drift가 확인되거나 batch read가 실패하면 fallback 정책에 따라 exact split 재검증 후 다시 스캔할 수 있습니다.
