...
반복되는 입력 힌트는 row마다 표시하지 않고 표 상단의 `검토 안내`에 한 번만 표시합니다. 개인정보 유형 패턴은 한글명 또는 원본 `pii_type` 값으로 입력할 수 있으며, HTML은 다운로드 시 한글명을 원본 `pii_type` 값으로 변환해 collector 스키마를 유지합니다.
