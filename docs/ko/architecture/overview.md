...
- `config/RulesetLoader.scala`: 기본/외부 ruleset과 suppression 로딩, regex 검증
...
- `model/Models.scala`: 결과/오류/PII 규칙 모델; `model/ScanPlanModels.scala`: 입력/그룹/스캔 계획 ADT
- `review/ReviewCollectCommand.scala`, `review/collect/`: response envelope 검증, collect lock, 누적 state 생성과 파일 교체
- `review/ReviewHtmlWriter.scala`, `review/ReviewHtmlRenderer.scala`: finding 구성, HTML 분할, 템플릿/스크립트 렌더링
- `src/main/resources/review/review.js`: 브라우저 입력 상태, 정렬, 검증, CSV/TSV 편집과 JSON 다운로드
...
1. CLI와 입력/출력/리뷰 경로 검증, SparkSession 생성
2. `--review-state-root`가 있으면 먼저 inbox를 수집하고 응답/lock 오류 시 스캔 시작 전 실패
3. 병렬도/Excel 설정, CSV/schema/parse cache, ignore와 명시적/state allowlist 준비
4. ruleset 로드, regex 사전 검증, ruleset/CLI suppression 병합
5. Hive JDBC 옵션이 모두 있으면 driver에서 `DBS`/`TBLS`/`SDS`를 1회 조회해 table `LOCATION` broadcast 인덱스 생성
6. 물리 파일 수집과 ignore 필터. 입력 루트가 정확한 Hive table `LOCATION`이고 ignore 매치가 없으면 table 스캔 계획을 만들고 파일 확장/스키마 split은 생략
7. 일반 파일 경로는 archive 엔트리와 workbook metadata 기반 시트 확장, direct compressed text-style input passthrough, magic-byte 판별, CSV dialect/text fallback 정규화, archive entry ignore 필터
8. layout 디렉토리를 정규화하고 `(directory, format)` 기준 그룹화, 대표 파일 스키마 샘플링, schema split과 디렉토리 식별자 승격 판정
9. `<output>/_progress-preparing.json` lock 획득 후 `<output>/_progress/<run_id>` 준비, stale progress 정리, heartbeat 시작과 plan 오류 기록
10. exact Hive table group이면 `spark.table`과 row sampling으로 테이블 스캔
11. 일반 sampled CSV/JSON group은 coordinator에서 exact split 재검증 후 재분류된 그룹 스캔
12. batch 가능한 그룹은 필요 시 bounded schema validation 후 batch scan. `xlsx` 등 나머지는 file scan. 물리 파일 경로에서는 선택적 file sampling, 그 외에는 결정적 row sampling 적용
13. 일반 group batch 실패 시 파일 fallback; sampled group의 schema drift/failure는 정책에 따라 exact split 재검증
14. recurring allowlist를 결과에 적용하고 group/file 완료 progress JSONL 기록. 실행 중 group/allowlist marker를 남기며 file marker는 설정 시에만 생성
15. progress JSONL merge, Hive 테이블 단위 최종 집계, 선택된 `scan_results`/`scan_errors` 포맷 저장
16. `--review-state-root`가 있으면 최종 결과로 리뷰 HTML 생성. 정상 완료 후 `_progress/<run_id>`와 staging 정리, SparkSession 종료
...
- suppression은 `DetectionMetrics.buildMetrics`에서 metric plan 생성 전에 적용해 제외된 `(column, pii_type)` 조합이 결과 row 자체를 만들지 않게 합니다.
...
- `--file-sample-ratio`는 물리 파일 batch scan과 file fallback scan에서 적용하고, 그룹 파일 수가 `--file-sample-min-files`보다 클 때만 `ceil(fileCount * ratio)` 수만큼 최소 1개 파일을 안정적인 해시 순위로 선택합니다.
- exact Hive table-root 스캔은 `spark.table`에 결정적 row sampling만 적용하고 file sampling 옵션은 사용하지 않습니다.
...
- 최종 출력 계약은 미지정 시 `parquet/scan_results`, `parquet/scan_errors`이고, CLI `--output-format`을 지정하면 선택한 `parquet/...`, `csv/...`, `excel/*.xlsx`만 생성합니다.
...
- file in-flight marker는 기본 off이며 `spark.privyspark.progress.fileMarker.enabled=true`로 켤 수 있습니다.
...
- `_progress`는 다음 실행 시작 시 stale 여부를 판정해 정리합니다. shutdown hook은 사용하지 않으며, 이전 완료 범위를 건너뛰는 checkpoint resume 기능도 없습니다.
...
sequenceDiagram
  ...
  opt reviewStateRoot 설정 시
    PrivySparkApp->>ReviewCollectCommand: inbox 검증과 state 수집
  end
