...
- When the input `--path` exactly matches a registered table-level `LOCATION`, that scan group is read through `spark.table("db.table")`, and both raw progress and final results use the table-root identifier. If Spark Catalog cannot resolve the same `db.table`, the failure is recorded in `scan_errors`.
