...
- A representative-file schema sample is used to refine `schemaSignature`; exact split remains for JSON sampled groups and fallback paths that need per-file schema confirmation.
...
- Sampled JSON groups are revalidated with exact split before scanning.
- Sampled batch-capable CSV, Parquet, ORC, and Avro groups take the batch path first and keep file-level identifiers when that batch scan succeeds.
- CSV exact split, when reached through fallback or explicit exact paths, also rechecks header drift.

This separate schema sampling phase exists for two reasons. First, directory-level aggregation must not hide schema drift. Second, reading every file upfront for exact split would inflate pre-scan cost, so the representative-file path trades cost against correctness and lets non-JSON small-file groups use the cheaper batch route first.
...
- Sampled JSON multi-file groups are exact-split revalidated before scanning.
- Sampled non-JSON batch-capable groups are batch-scanned first; if the batch read fails, the fallback policy may exact-split revalidate before rescanning.
