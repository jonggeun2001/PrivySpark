...
- Groups with confirmed identical schemas and no pre-scan errors may promote to directory identifiers. Single-file directories below the input root can also qualify; a single file at the input root and logical inputs keep their identifiers. See the [identifier rules](reports-and-errors.md#file_identifier-rules).
- Archive entries and workbook sheets always keep their logical identifiers.

An input root that exactly matches a Hive Metastore table `LOCATION`, with no ignore matches, takes a separate table path. It reads with `spark.table` instead of file expansion/schema split and applies row sampling only. See [Hive table lookup](../operations/execution.md#hive-table-lookup).
