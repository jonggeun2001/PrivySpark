...
- `--review-state-root` automatically collects `inbox/*.json` before the scan starts, updates cumulative offline-review state, applies the state allowlist, and writes `<output>/review/review.html` by default. Review HTML files are capped at 2MiB each; oversized reviews are split into a `review.html` index and `review-part-*.html` part files. `--review-html-dir` can point the review file output directory to a separate absolute path or URI.
...
- `--sample-ratio` deterministically samples by a hash of all row values. The same values and column order produce the same selection; exact row counts are not guaranteed.
- `--file-sample-ratio` selects a stable hash-ranked subset of files inside both batch scans and file-fallback scans.
- File sampling only applies when the group has more files than `--file-sample-min-files`. Hive exact table-root scans use row sampling only.
...
- Output formats: select Parquet, CSV, and/or Excel. Omit `--output-format` for Parquet only; repeat it to write the selected formats together
- Offline review, when enabled: `<output>/review/review.html` by default, or `review.html` under the directory configured by `--review-html-dir`; reviews over 2MiB also create `review-part-*.html` files in the same directory
...
- Offline review collection and cumulative state: [offline-review-collector.md](offline-review-collector.md)
