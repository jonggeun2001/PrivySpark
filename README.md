...
- 외부 규칙 파일 기반 정규식 탐지 (선택적 `column_hints` 기반 metric 축소 + 배치 집계 + 메트릭 50,000 초과 시 소배치 폴백)
...
`column_hints`는 선택 사항입니다. 지정하면 컬럼명에 해당 힌트가 포함된 컬럼에만 규칙을 적용하고, 생략하면 기존처럼 모든 컬럼을 검사합니다.
