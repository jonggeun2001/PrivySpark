...
서버 없이 담당자 검토를 받는 흐름은 스캔에 `--review-state-root`를 추가해 컬럼 헤더 정렬이 가능한 `review.html`을 만들고, 회수한 JSON을 `<review-state-root>/inbox`에 둔 뒤 다음 명령으로 반영합니다. HTML을 scan output 밖에 배치해야 하면 scan 실행에 `--review-html-dir /abs/reviews`를 추가합니다. 같은 finding이 정탐으로 계속 검출되면 다음 `review.html`의 `기존 조치 상태` 컬럼에 이전 조치 계획과 예정일이 표시됩니다.
