...
- `src/test/scala/io/github/jonggeun2001/privyspark`: Scala/Spark 테스트
- `src/test/js/review.test.cjs`: Node.js 내장 test runner로 실제 `review.js` 검증
...
- `bash scripts/verify-worktree.sh`: 표준 전체 검증(Scala/Spark + JavaScript, JDK 17 및 Node.js 18 이상)
- `./gradlew test`: Scala/Spark 테스트만 실행
...
- 최소 포함 항목:
  ...
  - 규칙셋 로드 실패/성공
  - 포맷 미지원 파일 오류 처리
- 전체 실행: `bash scripts/verify-worktree.sh`. Scala만 `./gradlew test`, 브라우저 로직만 `node --test src/test/js/*.test.cjs`로 실행합니다.
...
- 결과물: `privyspark-<tag>-all.jar`, `privyspark-<tag>-all.jar.sha256`, `default-rules.yaml`, `privyspark-<tag>-sample-datasets.zip`, `privyspark-<tag>-review-response-example.html`, `privyspark-<tag>-review-response-viewer.html`을 GitHub Release에서 다운로드 가능
...
- 리포트에 PII 원문 필드를 불필요하게 추가하지 않습니다. 현행 `scan_results`는 검출 조각 1건과 앞뒤 최대 50자 문맥을 저장하므로, 문서/예제에는 합성 데이터를 사용하고 원문 샘플을 확대하지 않습니다. `--review-sample-mode`는 HTML과 그 내보내기에만 적용하며 원본 리포트를 마스킹하지 않습니다.
