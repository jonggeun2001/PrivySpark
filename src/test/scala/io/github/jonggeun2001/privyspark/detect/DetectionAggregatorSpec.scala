...
import org.apache.spark.sql.functions.{col, sha2, trim, upper, when}
...
class DetectionAggregatorSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("default rules exclude complete SHA-256 values generated by Spark from counts and samples") {
    // These deterministic inputs produce embedded driver, phone, card, foreign, and resident
    // number witnesses; seed 40 also preserves the earlier compact resident-number regression.
    val lowercaseHashes = Seq(105, 178, 231, 1241, 1317, 40)
      .map(index => s"privyspark-sha256-probe-$index")
      .toDF("seed")
      .select(sha2(col("seed"), 256).as("notes"))
    val df = lowercaseHashes.union(lowercaseHashes.select(upper(col("notes")).as("notes")))
    val rules = RulesetLoader.load("default")

    val matchCounts = DetectionAggregator.aggregate(df, rules)
    val samples = DetectionAggregator.sampleMatches(df, rules, matchCounts)

    assert(matchCounts.isEmpty)
    assert(samples.isEmpty)
    assert(DetectionAggregator.countNonEmpty(df, Seq("notes")) == Map("notes" -> 12L))
  }

  test("default rules preserve real phone samples and non-empty denominators beside excluded SHA-256 values") {
    val hash = Seq("privyspark-sha256-probe-178").toDF("seed")
      .select(sha2(col("seed"), 256)).as[String].head()
    val df = Seq(
      ("alpha.csv", hash),
      ("alpha.csv", "010-1234-5678"),
      ("alpha.csv", null.asInstanceOf[String]),
      ("alpha.csv", ""),
      ("alpha.csv", " "),
      ("beta.csv", hash)
    ).toDF("file_id", "notes")
    val rules = RulesetLoader.load("default").filter(_.piiType == "phone_number")

    val matchCounts = DetectionAggregator.aggregate(df, rules)
    val samples = DetectionAggregator.sampleMatches(df, rules, matchCounts)
    val fileMatchCounts = DetectionAggregator.aggregateByFile(df, "file_id", rules)
    val fileSamples = DetectionAggregator.sampleMatchesByFile(df, "file_id", rules, fileMatchCounts)

    assert(sortByKey(matchCounts) == Seq(MatchCount("notes", "phone_number", 1L)))
    assert(samples(matchCounts.head.metricAlias).sampleMatchedFragment == "010-1234-5678")
    assert(samples(matchCounts.head.metricAlias).sampleRawValue == "010-1234-5678")
    assert(sortByFileKey(fileMatchCounts) == Seq(FileMatchCount("alpha.csv", "notes", "phone_number", 1L)))
    assert(fileSamples.size == 1)
    assert(fileSamples(("alpha.csv", fileMatchCounts.head.metricAlias)).sampleMatchedFragment == "010-1234-5678")
    assert(DetectionAggregator.countNonEmpty(df, Seq("notes")) == Map("notes" -> 3L))
    assert(DetectionAggregator.countNonEmptyByFile(df, "file_id", Seq("notes")) ==
      Map(("alpha.csv", "notes") -> 2L, ("beta.csv", "notes") -> 1L))

    val fullColumnRules = rules.map(_.copy(matchType = PiiRuleMatchType.FullColumn))
    val fullColumnCounts = DetectionAggregator.aggregate(df, fullColumnRules)
    val fullColumnSamples = DetectionAggregator.sampleMatches(df, fullColumnRules, fullColumnCounts)
    assert(sortByKey(fullColumnCounts) == Seq(MatchCount("notes", "phone_number", 1L)))
    assert(fullColumnSamples(fullColumnCounts.head.metricAlias).sampleMatchedFragment == "010-1234-5678")
  }

  test("default rules still inspect SHA-256 fragments when the raw value is not exactly 64 hex characters") {
    val hash = Seq("privyspark-sha256-probe-178").toDF("seed")
      .select(sha2(col("seed"), 256)).as[String].head()
    val df = Seq(
      s" $hash ",
    ...
    assert(sortByKey(matchCounts) == Seq(MatchCount("notes", "phone_number", 5L)))
    assert(samples(matchCounts.head.metricAlias).sampleMatchedFragment == "0172352458")
  }

  test("custom regex can still count and sample complete SHA-256 values in both match modes") {
    val df = Seq("privyspark-sha256-probe-178").toDF("seed")
      .select(sha2(col("seed"), 256).as("notes"))
    val hash = df.as[String].head()
    val rules = Seq(
      PiiRule("custom_hash", "[0-9a-f]{64}"),
      PiiRule("custom_hash_full", "[0-9a-f]{64}", matchType = PiiRuleMatchType.FullColumn)
    )

    val matchCounts = DetectionAggregator.aggregate(df, rules)
    val samples = DetectionAggregator.sampleMatches(df, rules, matchCounts)

    assert(sortByKey(matchCounts) == Seq(
      MatchCount("notes", "custom_hash", 1L),
      MatchCount("notes", "custom_hash_full", 1L)
    ))
    assert(samples.size == 2)
    assert(matchCounts.forall(count => samples(count.metricAlias).sampleMatchedFragment == hash))
    assert(matchCounts.forall(count => samples(count.metricAlias).sampleRawValue == hash))
  }
