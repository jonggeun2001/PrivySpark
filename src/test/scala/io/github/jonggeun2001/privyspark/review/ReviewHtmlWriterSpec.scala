...
class ReviewHtmlWriterSpec extends AnyFunSuite {
  ...
  test("write exposes CSV review workflow and does not create review workbook") {
    ...
    assert(html.contains("""<textarea id="pasteReviewCsv" rows="3" spellcheck="false"></textarea>"""))
    assert(html.contains("""<button type="button" id="importPastedReviewCsv">붙여넣은 CSV 반영</button>"""))
    ...
    assert(html.contains("CSV 파일 업로드와 붙여넣기는 따옴표로 감싼 쉼표와 줄바꿈을 유지합니다."))
    assert(html.contains("CSV 임포트는 <code>finding_key</code> 기준으로 판정/사유/계획/예정일만 반영합니다."))
    ...
    assert(html.contains("function importReviewCsvText(text)"))
    assert(html.contains("const rows = parseDelimitedText(text, ',').filter"))
    assert(html.contains("function handleReviewCsvFile(event)"))
    assert(html.contains("function importPastedReviewCsv()"))
    ...
    assert(html.contains("document.getElementById('importPastedReviewCsv').addEventListener('click', importPastedReviewCsv);"))
  }
