...
class ReleaseArtifactWorkflowSpec extends AnyFunSuite {
  ...
  test("offline review response HTML example is self-contained and downloads response JSON") {
    ...
    assert(html.contains("<th>경로 / Hive</th>"))
    assert(html.contains("<th>컬럼 / PII</th>"))
    assert(html.contains("<th>지표</th>"))
    assert(html.contains("<th>샘플</th>"))
    assert(html.contains("<th>판정</th>"))
    assert(html.contains("<th>사유 / 계획</th>"))
