...
class ReleaseArtifactWorkflowSpec extends AnyFunSuite {
  ...
  test("review HTML supports CSV file import and Excel TSV clipboard paste") {
    val template = readResource("review/review.html.template")
    val script = readResource("review/review.js")

    assert(template.contains("<button type=\"button\" id=\"downloadReviewCsv\">엑셀 편집용 CSV 다운로드</button>"))
    assert(template.contains("""<input id="importReviewCsv" type="file" accept=".csv,text/csv,text/plain">"""))
    assert(template.contains("""Excel 셀 복사 후 붙여넣기 <textarea id="pasteReviewTsv""""))
    assert(template.contains("""<textarea id="pasteReviewTsv" rows="3" spellcheck="false"></textarea>"""))
    assert(template.contains("""<button type="button" id="importPastedReviewTsv">반영</button>"""))
    assert(template.contains("사내 보안 솔루션이 CSV 파일을 암호화한 경우 반드시 암호화 해제한 CSV 파일을 불러옵니다."))
    assert(template.contains("Excel에서 전체 복사한 뒤 TSV 붙여넣기 칸에 붙여넣고 버튼을 누르면 아래 표에 반영됩니다."))
    assert(template.contains("CSV 파일 업로드는 따옴표로 감싼 쉼표와 줄바꿈을 유지하고, TSV 붙여넣기는 탭과 줄바꿈을 기준으로 반영합니다."))
    assert(template.contains("Excel이 줄바꿈 포함 셀을 큰따옴표로 감싼 경우 줄바꿈은 셀 내용으로 유지됩니다."))
    assert(script.contains("const ReviewCsvHeaders = ["))
    assert(script.contains("'검출샘플(검출값/데이터)'"))
    assert(script.contains("'검출비율(%)'"))
    assert(script.contains("function neutralizeCsvFormulaValue(value)"))
    assert(script.contains("""/^[=+\-@]/.test(text)"""))
    assert(script.contains("function escapeCsvCell(value)"))
    assert(script.contains("const text = neutralizeCsvFormulaValue(value);"))
    assert(script.contains("""if (/[,\t\r\n"]/.test(text))"""))
    assert(script.contains("function reviewCsvRows()"))
    assert(script.contains("function downloadReviewCsv()"))
    assert(script.contains(".join('\\r\\n');"))
    assert(script.contains("function parseDelimitedText(text, delimiter)"))
    assert(script.contains("if (ch === '\"')"))
    assert(script.contains("if (text[index + 1] === '\\n')"))
    assert(script.contains("function normalizeImportedDecision(value)"))
    assert(script.contains("function importReviewDelimitedText(text, delimiter, formatLabel)"))
    assert(script.contains("const rows = parseDelimitedText(text, delimiter).filter"))
    assert(script.contains("importReviewDelimitedText(text, ',', 'CSV');"))
    assert(script.contains("importReviewDelimitedText(text, '\\t', 'TSV');"))
    assert(script.contains("function handleReviewCsvFile(event)"))
    assert(script.contains("function importPastedReviewTsv()"))
  }
  ...
  test("offline review response HTML example is self-contained and downloads response JSON") {
    ...
    assert(html.contains("function firstEvidenceSampleValue(finding, fieldName)"))
    assert(html.contains("sample_matched_fragment: firstEvidenceSampleValue(finding, 'sample_matched_fragment')"))
    assert(html.contains("sample_raw_value: firstEvidenceSampleValue(finding, 'sample_raw_value')"))
  ...
  test("offline review response JSON viewer is self-contained and loads local response files") {
    ...
    assert(html.contains(".high-ratio-false-positive"))
    assert(html.contains("function detectionRatioPercent(response)"))
    assert(html.contains("function isHighRatioFalsePositive(response)"))
    assert(html.contains("let responseSortState = { key: 'target', direction: 'asc' };"))
    assert(html.contains("function responseTargetSortText(response)"))
    assert(html.contains("function getResponseSortValue(response)"))
    assert(html.contains("function sortResponses(responses)"))
    assert(html.contains("function updateResponseSortHeaders()"))
    assert(html.contains("검출비율 50% 초과 오탐"))
    assert(html.contains("data-risk=\"high_false_positive\""))
    assert(html.contains("const highRatioFalsePositiveCount = responses.filter(isHighRatioFalsePositive).length;"))
    assert(html.contains("const clearRenderedState ="))
    assert(html.contains("data-field=\"decision\""))
    assert(html.contains("""<th data-sort-key="target" aria-sort="none"><button type="button" class="sort-button">대상 <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("""<th data-sort-key="detection_ratio" aria-sort="none"><button type="button" class="sort-button">검출비율 <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("<th>샘플/추출값</th>"))
    assert(!html.contains("<th>근거 식별자</th>"))
    assert(html.contains("response.sample_matched_fragment"))
    assert(html.contains("response.sample_raw_value"))
    assert(html.contains("<div class=\"sample-label\">추출값</div>"))
    assert(html.contains("<div class=\"sample-label\">샘플</div>"))
    assert(!html.contains("${field(response.finding_key)}\n${field(response.finding_hash)}"))
    assert(!html.contains("http://"))
    assert(!html.contains("https://"))
  }
