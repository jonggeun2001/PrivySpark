...
class ReleaseArtifactWorkflowSpec extends AnyFunSuite {
  ...
  test("review HTML supports Excel-editable CSV export and decrypted CSV import") {
    ...
    assert(template.contains("""<textarea id="pasteReviewCsv" rows="3" spellcheck="false"></textarea>"""))
    assert(template.contains("""<button type="button" id="importPastedReviewCsv">붙여넣은 CSV 반영</button>"""))
    assert(template.contains("사내 보안 솔루션이 CSV 파일을 암호화한 경우 반드시 암호화 해제한 CSV 파일을 불러옵니다."))
    assert(template.contains("CSV 파일 업로드와 붙여넣기는 따옴표로 감싼 쉼표와 줄바꿈을 유지합니다."))
    ...
    assert(script.contains("function importReviewCsvText(text)"))
    assert(script.contains("const rows = parseDelimitedText(text, ',').filter"))
    assert(script.contains("function handleReviewCsvFile(event)"))
    assert(script.contains("function importPastedReviewCsv()"))
  }
