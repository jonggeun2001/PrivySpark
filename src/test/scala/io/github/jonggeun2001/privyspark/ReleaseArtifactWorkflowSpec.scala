...
class ReleaseArtifactWorkflowSpec extends AnyFunSuite {
  ...
  test("offline review response HTML example is self-contained and downloads response JSON") {
    ...
    assert(html.contains("exact: 이 경로의 finding들을 각각 제외"))
    ...
    assert(html.contains("""<th scope="col" data-sort-key="hive" aria-sort="none"><button type="button" class="sort-button">Hive <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("""<th scope="col" data-sort-key="column" aria-sort="none"><button type="button" class="sort-button">컬럼 <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("""<th scope="col" data-sort-key="pii" aria-sort="none"><button type="button" class="sort-button">PII <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("""<th scope="col" data-sort-key="sampled_row_count" aria-sort="none"><button type="button" class="sort-button">sampled_row_count <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("""<th scope="col" data-sort-key="match_count" aria-sort="none"><button type="button" class="sort-button">match_count <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("""<th scope="col" data-sort-key="non_empty_match_ratio" aria-sort="none"><button type="button" class="sort-button">non_empty_match_ratio <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    ...
    assert(html.contains("""<th scope="col" data-sort-key="file_identifier_pattern" aria-sort="none"><button type="button" class="sort-button">file_identifier_pattern <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("""<th scope="col" data-sort-key="column_name_pattern" aria-sort="none"><button type="button" class="sort-button">column_name_pattern <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("""<th scope="col" data-sort-key="pii_type_pattern" aria-sort="none"><button type="button" class="sort-button">pii_type_pattern <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    assert(html.contains("""<th scope="col" data-sort-key="expires_at" aria-sort="none"><button type="button" class="sort-button">pattern expires_at <span class="sort-indicator" aria-hidden="true"></span></button></th>"""))
    ...
    assert(html.contains("""<td class="metric-cell">${escapeHtml(group.sampled_row_count)}</td>"""))
    assert(html.contains("""<td class="metric-cell">${escapeHtml(group.match_count)}</td>"""))
    assert(html.contains("""<td class="metric-cell">${escapeHtml(group.non_empty_match_ratio)}</td>"""))
    ...
    assert(html.contains("group.finding_indexes.push(findingIndex);"))
    ...
    assert(html.contains("FINDING_GROUPS.flatMap((group, groupIndex) =>"))
    assert(html.contains("group.findings.map(finding => Object.assign({"))
