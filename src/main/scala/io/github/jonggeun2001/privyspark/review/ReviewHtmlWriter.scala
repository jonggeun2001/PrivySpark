      ...
      return `
        ...
        <td>
          <div class="decision-toggle" role="group" aria-label="판정" data-validation-field="decision" aria-invalid="false">
            <button type="button" class="decision-button" data-index="$${index}" data-decision-button="false_positive" aria-pressed="false"$${finding.fingerprint_complete ? '' : ' disabled'}>오탐</button>
        ...
        <td class="date-cell">
          <div class="decision-fields" data-decision-section="true_positive">
            <input data-index="$${index}" data-field="action_due_date" type="date" aria-label="조치 예정일" placeholder="YYYY-MM-DD">
          </div>
        </td>`;
    }
