...
- Use the false-positive scope column only for false positives: `exact` for fingerprint-checked findings, `pattern` for repeated false positives. Pattern fields and expiry date appear inside that scope column when `pattern` is selected.
...
- False-positive reason and true-positive action-plan fields are shown based on the selected decision.
